...
// Total count
$totalQuery = "SELECT COUNT(*) as total FROM movies $searchSQL";
$totalStmt = $con->prepare($totalQuery);
if ($types) $totalStmt->bind_param($types, ...$params);
$totalStmt->execute();
$totalMovies = $totalStmt->get_result()->fetch_assoc()['total'];
$totalPages = ceil($totalMovies / $perPage);
$totalStmt->close();

// Fetch movies
$query = "SELECT id, title, description, cover_image FROM movies $searchSQL ORDER BY created_at DESC LIMIT ? OFFSET ?";
...
<div class="navbar">
    ...
    <div class="right">
        ...
        <?php endif; ?>
        <input type="text" placeholder="Search title..." value="<?= htmlspecialchars($search) ?>" onkeyup="if(event.key === 'Enter') window.location.href='movies.php?s=' + encodeURIComponent(this.value)">
        <button class="dark-toggle" onclick="toggleDarkMode()">🌙</button>
    </div>
</div>

<div class="section">
    <h2 style="text-align:center; margin-bottom:30px;">Movies</h2>
    <div class="grid">
    ...
    <div class="pagination">
        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
            <button onclick="location.href='movies.php?page=<?= $i ?>&s=<?= urlencode($search) ?>'" 
                class="<?= $i == $page ? 'active' : '' ?>">
                <?= $i ?>
            </button>
        <?php endfor; ?>
