...
$totalQuery = $con->query("SELECT COUNT(*) AS total FROM movies");
$totalMovies = $totalQuery->fetch_assoc()['total'];
$totalPages = ceil($totalMovies / $limit);

$movies = $con->query("SELECT * FROM movies ORDER BY created_at DESC LIMIT $limit OFFSET $offset");

$featured = $con->query("SELECT movie_id FROM featured_homepage");
$featured_ids = [];
while ($row = $featured->fetch_assoc()) {
    $featured_ids[] = $row['movie_id'];
}
?>
<!DOCTYPE html>
<html lang="en">
...
<div id="sidebar" class="sidebar">
  ...
  <div class="right-btns">
    <a href="add_movie.php">
      <button class="btn-collection">➕ Create Movie</button>
    </a>
  </div>
</div>
      ...
      <tbody>
        <?php while ($row = $movies->fetch_assoc()): ?>
          <tr>
            ...
            <td><?= $row['tier_access'] === 'free' ? 'All' : ucfirst($row['tier_access']) ?></td>
            <td><?= in_array($row['id'], $featured_ids) ? '⭐' : '' ?></td>
            <td>🎞️ Video</td>

            <td>
              <a href="add_movie.php?edit=<?= $row['id'] ?>">✏️ Edit</a>
              <a href="?delete=<?= $row['id'] ?>" onclick="return confirm('Delete this movie?')">🗑️ Delete</a>
            </td>
          </tr>
        <?php endwhile; ?>
      </tbody>
